...
if (!url || !key) {
  ...
  fs.writeFileSync('js/config.js',
    `window.supabase = { from: () => { throw new Error('Supabase env vars not set in Vercel. Add SUPABASE_URL and SUPABASE_ANON_KEY in Vercel → Settings → Environment Variables.'); } };\n`
  );
